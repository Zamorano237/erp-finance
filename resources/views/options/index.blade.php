    ...
    <div class="options-kpi-card">
        <div class="options-kpi-top">
            <div>
                ...
                <div class="options-kpi-value">{{ $dashboard['lists_count'] ?? 0 }}</div>
            </div>
    ...
    <div class="options-kpi-card">
        <div class="options-kpi-top">
            <div>
                ...
                <div class="options-kpi-value">{{ $dashboard['items_count'] ?? 0 }}</div>
            </div>
    ...
    <div class="options-kpi-card">
        <div class="options-kpi-top">
            <div>
                ...
                <div class="options-kpi-value">{{ $dashboard['active_items_count'] ?? 0 }}</div>
            </div>
    ...
    <div class="options-kpi-card">
        <div class="options-kpi-top">
            <div>
                ...
                <div class="options-kpi-value">{{ $dashboard['inactive_items_count'] ?? 0 }}</div>
            </div>
...
<div class="options-grid">
    @forelse($optionLists as $optionList)
        <div class="options-card">
            <div class="options-card-head">
                <div>
                    <h3 class="options-card-title">{{ $optionList->label }}</h3>
                    <div class="options-card-subtitle">Code : {{ $optionList->code }}</div>
                </div>

                <span class="badge info">{{ $optionList->items->count() }} valeur(s)</span>
            </div>

            <div class="options-items-list js-options-list" data-list-id="{{ $optionList->id }}">
                @forelse($optionList->items as $item)
                    <div class="options-item-row" draggable="true" data-item-id="{{ $item->id }}">
                        ...
                        <div class="options-item-actions">
                            <button
                                type="button"
                                class="btn btn-light btn-sm options-toggle-btn {{ $item->is_active ? 'is-on' : 'is-off' }} js-option-toggle"
                                data-list-id="{{ $optionList->id }}"
                                data-item-id="{{ $item->id }}"
                                title="{{ $item->is_active ? 'Désactiver' : 'Activer' }}"
                            >
                                <i data-lucide="{{ $item->is_active ? 'toggle-right' : 'toggle-left' }}"></i>
                            </button>
            ...
            <form method="POST" action="{{ route('options.items.store', $optionList) }}">
                @csrf
